...
>
源代码(.java)通过编译器编译为字节码(.class)，再通过类加载子系统进行加载到JVM中运行。最后由JVM将对应指令翻译成机器语言至操作系统完成操作。
>

### 跨语言在JVM运行
<img src="..\..\..\picture service\JVM\1.png">

### 字节码文件解析
class文件本质上是一个以8位字节为基础单位的二进制流，各个数据项目严格按照顺序紧凑的排列在class文件中。jvm根据其**特定的规则**解析该二进制数据，从而得到相关信息。Class文件采用一种伪结构来存储数据，它有两种类型：无符号数和表。

#### Class文件的结构属性
<img src="..\..\..\picture service\JVM\2.png">
