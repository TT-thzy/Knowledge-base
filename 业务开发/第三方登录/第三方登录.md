...
1. 客户端对用户发起获取第三方应用访问的权限的;
2. 用户输入认证信息后，客户端发送获取第三方应用认证凭证的请求;
3. 服务端获取请求后，校验用户输入的认证信息是否合法，如何合法，服务端返回一个授权码给客户端;
4. 客户端获取到授权码后，发送通过授权码换取认证凭证的请求;
5. 服务端获取到授权码后，校验授权码的合法性后，发放相应权限的凭证;
6. 客户端收到凭证后，就可通过该凭证访问服务端的资源;
